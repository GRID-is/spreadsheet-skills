...
# inside the sandbox. __ENGINE_SPEC__ is replaced by scripts/run-evals.mjs with
# the version range this repo's package.json pins.
set -euo pipefail
npm init -y >/dev/null
npm install --no-audit --no-fund --silent "@grid-is/spreadsheet-engine@__ENGINE_SPEC__"
